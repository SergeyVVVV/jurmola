 ...
 * Fixes broken covers, duplicate covers, and ensures one image per article.
 */
...
const ids: { id: number; pos: number }[] = [];
let m;
while ((m = idRegex.exec(s)) !== null) {
  ids.push({ id: parseInt(m[1], 10), pos: m.index });
}

const images: { url: string; full: string; start: number; end: number }[] = [];
...
const pairs: { id: number; url: string; full: string; start: number; end: number }[] = [];
